...
Create a `.env` file in the project root directory. Copy `docs/.env.example` as a template:
...
│   │   │   └── vehicle-images/# Image proxy endpoint
...
│   │   └── vehicle.ts         # Product/vehicle types
...
**`/api/vehicle-images/route.ts`**: Image proxy endpoint
...
- Lists compatibility types (socket, PCIe, etc.)
...
```typescript
{
  ...
  vehicles: Product[];            // Recommended products

...
try:
    from .scrape_compatibility_data import CompatibilityScraper
    from .normalize_attributes import normalize_attribute_value
except ImportError:
    CompatibilityScraper = None  # type: ignore
    normalize_attribute_value = None  # type: ignore
...
class ComponentRecord:
    ...
    def to_payload(self, scraped_attributes: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        """
        Convert component to Neo4j payload.
        Attributes are stored as properties on the node, not separate nodes.
        """
        stats = self.price_summary
        
        # Build properties dictionary from attributes
        properties: Dict[str, Any] = {}
        
        # Add normalized attributes as properties
        for attr in self.attributes:
            # Normalize attribute key to property name
            prop_key = attr.kind.lower().replace(" ", "_")
            if prop_key not in properties:
                properties[prop_key] = []
            properties[prop_key].append(attr.value)
        
        # Merge with scraped attributes if available
        if scraped_attributes:
        ...
        for key in list(properties.keys()):
            if isinstance(properties[key], list) and len(properties[key]) == 1:
                properties[key] = properties[key][0]
        
        return {
            "slug": self.slug,
            "name": self.name,
            "product_type": self.component_type,  # Changed from "type"
            "brand": self.manufacturer,  # Changed from "manufacturer"
            "model": self.model,
            "sellers": sorted(self.sellers) if self.sellers else [],  # Array of sellers (empty if none)
            "price_min": stats["min"],
            "price_max": stats["max"],
            "price_avg": stats["avg"],
            **properties,  # Spread all attribute properties
        }
...
# Data sources that should NOT be treated as sellers
NON_SELLER_SOURCES = {
    "rapidapi",
    "rapid api",
    "api",
...
# Generic words that are NOT valid manufacturer names
GENERIC_MANUFACTURER_WORDS = {
    "seller",
    "inc",
    "inc.",
    "ltd",
...
def is_retailer(token: str) -> bool:
    ...
    if not normalized:
        return False
    # Check if it's a known non-seller source first
    if normalized in NON_SELLER_SOURCES:
        return False
...
def is_valid_seller(seller: str) -> bool:
    """Check if a seller string represents a valid retailer."""
    if not seller:
        return False
    normalized = normalize_whitespace(seller).lower()
    # Filter out non-seller sources
    if normalized in NON_SELLER_SOURCES:
        return False
    # Check if it's a known retailer
    if is_retailer(normalized):
        return True
    # If it contains retailer keywords, it might be valid
...
def extract_sellers_from_name(name: str) -> tuple[str, Set[str]]:
    """Extract seller names from product name and return cleaned name + sellers found.
    
    Returns:
        tuple: (cleaned_name, set_of_sellers_found)
    """
    sellers_found: Set[str] = set()
    current = name.strip()
    ...
    for _ in range(4):
        ...
        # Check if prefix is a retailer
        if is_retailer(prefix):
            sellers_found.add(normalize_whitespace(prefix))
            current = rest
            continue
        ...
        if sub_parts and all(is_retailer(part) for part in sub_parts):
            for part in sub_parts:
                sellers_found.add(normalize_whitespace(part))
            current = rest
            continue
        break
    
    cleaned_name = current or name
    return cleaned_name, sellers_found


def strip_seller_prefix(name: str) -> str:
    """Legacy function - use extract_sellers_from_name instead for new code."""
    cleaned_name, _ = extract_sellers_from_name(name)
    return cleaned_name
...
def clean_manufacturer(name: Optional[str], fallback: Optional[str]) -> Optional[str]:
    """Clean manufacturer name by removing seller prefixes and generic words.
    
    Returns the cleaned manufacturer name without sellers.
    If the result is empty or only contains generic words, returns fallback.
    """
    if not name:
        return fallback
    ...
    # Extract sellers and get cleaned name
    cleaned, _ = extract_sellers_from_name(candidate)
    
    # If cleaned is empty or same as original, check if it's all retailers
    if not cleaned or cleaned == candidate:
        # Check if it's all retailers
    ...
    # Check if cleaned result is just generic words (not a real manufacturer)
    cleaned_lower = cleaned.lower().strip()
    if cleaned_lower in GENERIC_MANUFACTURER_WORDS:
        return fallback
    
    # Check if cleaned contains only generic words
    cleaned_tokens = [t.strip().lower() for t in re.split(r"\s+", cleaned) if t.strip()]
    if cleaned_tokens and all(token in GENERIC_MANUFACTURER_WORDS for token in cleaned_tokens):
        return fallback
    
    # If we have a meaningful manufacturer name, return it
    return cleaned if cleaned != candidate else candidate
...
def parse_gpu(row: Dict[str, Any], canonical_type: str) -> Optional[ComponentRecord]:
    ...
    product_name, name_sellers = extract_sellers_from_name(raw_name)
    
    # Brand is guessed from product name only (no manufacturer field in database)
    ...
    # Brand is guessed from product name, or use vendor from metadata
    final_brand = brand_guess or vendor or "Generic"
    vendor_name = final_brand  # Use same brand for slug
    ...
    component = ComponentRecord(
        ...
        name=display_name,
        manufacturer=final_brand,  # Brand from product name
        model=model,
        sellers=name_sellers.copy(),  # Add sellers extracted from name
        metadata={
...
def parse_psu(row: Dict[str, Any], canonical_type: str) -> Optional[ComponentRecord]:
    ...
    # Brand is guessed from product name only
    final_brand = brand_guess or "Generic"
    slug = slugify(f"{final_brand} {product_name}")
    display_name = build_display_name(product_name, canonical_type)
    ...
    component = ComponentRecord(
        ...
        component_type=canonical_type,
        name=display_name,
        manufacturer=final_brand,  # Brand from product name takes priority
        model=base_model,
        sellers=name_sellers.copy(),  # Add sellers extracted from name
        metadata={
            "wattage": wattage,
...
def parse_cpu(row: Dict[str, Any], canonical_type: str) -> Optional[ComponentRecord]:
    ...
    tdp = tdp_val or 95.0
    architecture = arch or brand_guess or "CPU"
...
def consolidate_components(
    ...
    for row in rows:
        ...
        # Extract seller - prefer "seller" field, but validate it's actually a seller
        seller = row.get("seller")
        if seller and is_valid_seller(str(seller)):
            component.sellers.add(normalize_whitespace(str(seller)))
        
        # Don't use "source" as seller - it's usually a data source like "rapidapi"
        # Only use it if it's actually a retailer name
        source = row.get("source")
        if source and source != seller and is_valid_seller(str(source)):
            component.sellers.add(normalize_whitespace(str(source)))
...
def build_electrical_edges(psus: Sequence[ComponentRecord], gpus: Sequence[ComponentRecord]) -> List[Dict[str, Any]]:
    ...
    for psu in psus:
        ...
        try:
            ...
            # Only create edges for realistic PSU wattages (350W - 2000W based on actual products)
            if psu_watts < 350 or psu_watts > 2000:
                continue
        except (ValueError, TypeError):
            continue
        for gpu in gpus:
            # Only create edges if GPU has explicit series match (not default/inferred)
            series_source = gpu.metadata.get("series_source")
            if series_source == "name_inferred":
                continue  # Skip GPUs with default values
            
            requirement = gpu.metadata.get("recommended_psu_watts")
            if not requirement:
                continue
            try:
                req_watts = float(requirement)
                # Only create edges for realistic GPU requirements (350W - 850W based on actual GPU series)
                if req_watts < 350 or req_watts > 850:
                    continue
            except (ValueError, TypeError):
                continue
            margin = psu_watts - req_watts
            # Require at least 50W margin for safety
            if margin < 50:
                continue
            edges.append(
                _wrap_edge(
...
def build_interface_edges(motherboards: Sequence[ComponentRecord], gpus: Sequence[ComponentRecord]) -> List[Dict[str, Any]]:
    ...
    for board in motherboards:
        ...
        if not board_pcie:
            continue
        try:
            board_pcie_val = float(board_pcie)
            # Only create edges for realistic PCIe versions (3.0-5.0 based on actual chipsets)
            # Exclude default 4.0 values when chipset is unknown
            if board_pcie_val < 3.0 or board_pcie_val > 5.0:
                continue
            # Skip if PCIe is default (4.0) and chipset is missing (indicates default assumption)
            chipset = board.metadata.get("chipset")
            if not chipset and board_pcie_val == 4.0:
                continue  # Likely a default value, not explicit
        except (ValueError, TypeError):
            continue
        for gpu in gpus:
            # Only create edges if GPU has explicit series match (not default/inferred)
            series_source = gpu.metadata.get("series_source")
            if series_source == "name_inferred":
                continue  # Skip GPUs with default PCIe values
            
            gpu_pcie = gpu.metadata.get("pcie_requirement")
            if not gpu_pcie:
                continue
            try:
                gpu_pcie_val = float(gpu_pcie)
                # Only create edges for realistic PCIe requirements (3.0-4.0 based on actual GPU series)
                if gpu_pcie_val < 3.0 or gpu_pcie_val > 4.0:
                    continue
            except (ValueError, TypeError):
                continue
            # Board PCIe version must be >= GPU requirement (PCIe is forward compatible)
            # Note: PCIe is also backward compatible, but we only model forward compatibility here
            if board_pcie_val + 1e-3 >= gpu_pcie_val:
                edges.append(
                    _wrap_edge(
                        board.slug,
                        gpu.slug,
                        {
                            "board_pcie": board_pcie_val,
                            "gpu_requirement": gpu_pcie_val,
                        },
                    )
                )
...
def build_cooling_edges(coolers: Sequence[ComponentRecord], cpus: Sequence[ComponentRecord]) -> List[Dict[str, Any]]:
    ...
    for cooler in coolers:
        ...
        if not support:
            continue
        try:
            support_watts = float(support)
            if support_watts <= 0:  # Validate reasonable TDP support
                continue
        except (ValueError, TypeError):
            continue
        sockets = {sock.lower() for sock in cooler.metadata.get("socket_support", [])}
        # Only create edges if cooler has socket support specified (more accurate)
        if not sockets:
            continue
        for cpu in cpus:
            ...
            if not required:
                continue
            try:
                req_watts = float(required)
                if req_watts <= 0:  # Validate reasonable TDP requirement
                    continue
            except (ValueError, TypeError):
                continue
            cpu_socket = cpu.metadata.get("socket")
            # Require socket match - CPU must have socket info and it must match cooler's supported sockets
            if not cpu_socket:
                continue
            if cpu_socket.lower() not in sockets:
                continue
            margin = support_watts - req_watts
            if margin < 0:
                continue
            edges.append(
                _wrap_edge(
                    ...
                    cpu.slug,
                    {
                        "cooler_support_watts": support_watts,
                        "cpu_requirement_watts": req_watts,
                        "margin_watts": round(margin, 1),
                    },
                )
            )
...
def upsert_components(
    ...
    scraper: Optional[CompatibilityScraper] = None
) -> None:
    """
    Upsert product nodes with all attributes as properties.
    No separate Attribute or Constraint nodes are created.
    """
    payloads = []
    for component in components:
        scraped_attrs = None
        if scraper:
            # Try to get cached scraped attributes
            cached = scraper.get_cached_attributes(component.slug)
            if cached:
                scraped_attrs = {}
                for attr in cached:
                    if attr.attribute_type not in scraped_attrs:
                        scraped_attrs[attr.attribute_type] = []
                    scraped_attrs[attr.attribute_type].append(attr.attribute_value)
                # Convert lists to single values
                for key in list(scraped_attrs.keys()):
                    if isinstance(scraped_attrs[key], list) and len(scraped_attrs[key]) == 1:
                        scraped_attrs[key] = scraped_attrs[key][0]
        
        payloads.append(component.to_payload(scraped_attrs))
    
    if not payloads:
        LOGGER.warning("No component payloads to upsert")
        return

    # Define excluded keys (core properties set explicitly)
    excluded_keys = ['slug', 'name', 'product_type', 'brand', 'model', 
                     'sellers', 'price_min', 'price_max', 'price_avg']
    ...
    LOGGER.info("Upserted %d product nodes with properties", len(payloads))


# Removed: upsert_attributes, upsert_constraints, gather_constraints, upsert_constraint_satisfaction
# Attributes and constraints are now stored as properties on Product nodes
# Compatibility is represented as edges between Product nodes


def upsert_compatibility_edges(driver, namespace: str, edge_specs: Sequence[Dict[str, Any]]) -> None:
    """
    Create typed compatibility edges between Product nodes.
    
    Each compatibility type gets its own relationship type (e.g., ELECTRICAL_COMPATIBLE_WITH, SOCKET_COMPATIBLE_WITH).
    Multiple relationship types can exist between the same two products.
    
    Example: A PSU and GPU can have:
    - ELECTRICAL_COMPATIBLE_WITH (power requirements)
    - And potentially other compatibility types
    
    Each relationship type is created independently, so products can have multiple
    compatibility edges representing different aspects of compatibility.
    """
    with driver.session() as session:
        for spec in edge_specs:
            relation_type: str = spec["type"]  # e.g., "ELECTRICAL_COMPATIBLE_WITH"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--enable-scraping", action="store_true", help="Enable web scraping for compatibility data (slower).")
    parser.add_argument("--step", type=int, choices=[1, 2, 3], help="Build step: 1=create nodes only, 2=scrape data, 3=update graph with scraped data")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    build_step = args.step if args.step else 1  # Default to step 1
    
    if build_step == 1:
        # STEP 1: Create nodes from database (no scraping)
        LOGGER.info("=" * 60)
        LOGGER.info("STEP 1: Creating product nodes from database")
        LOGGER.info("=" * 60)
        ...
        # Create nodes WITHOUT scraped data
        upsert_components(driver, args.namespace, all_components, scraper=None)
        
        # Create typed compatibility edges based on parsed attributes
        edge_specs = [
            {"type": "ELECTRICAL_COMPATIBLE_WITH", "edges": electrical_edges},
            {"type": "INTERFACE_COMPATIBLE_WITH", "edges": interface_edges},
            {"type": "SOCKET_COMPATIBLE_WITH", "edges": socket_edges},
            {"type": "RAM_COMPATIBLE_WITH", "edges": ram_board_edges},
            {"type": "FORM_FACTOR_COMPATIBLE_WITH", "edges": case_board_edges},
            {"type": "MEMORY_COMPATIBLE_WITH", "edges": ram_cpu_edges},
            {"type": "THERMAL_COMPATIBLE_WITH", "edges": cooling_edges},
        ]
        upsert_compatibility_edges(driver, args.namespace, edge_specs)
        
        driver.close()
        LOGGER.info("STEP 1 complete: Product nodes created from database")
        LOGGER.info("Next: Run step 2 to scrape compatibility data")
        
    elif build_step == 2:
        # STEP 2: Scrape compatibility data for all products
        LOGGER.info("=" * 60)
        LOGGER.info("STEP 2: Scraping compatibility data for all products")
        LOGGER.info("=" * 60)
        
        # Import step 2 script
        from .kg_step2_scrape import scrape_all_products
        
        all_components = [comp for comp_list in components.values() for comp in comp_list]
        scrape_all_products(
            components=all_components,
            compatibility_db=args.compatibility_db,
            use_llm=args.enable_scraping,
            limit=args.limit
        )
        LOGGER.info("STEP 2 complete: Compatibility data scraped and cached")
        LOGGER.info("Next: Run step 3 to update graph with scraped data")
        
    elif build_step == 3:
        # STEP 3: Update graph with scraped data
        LOGGER.info("=" * 60)
        LOGGER.info("STEP 3: Updating graph with scraped compatibility data")
        LOGGER.info("=" * 60)
        
        driver = ensure_driver(args.neo4j_uri, args.neo4j_user, args.neo4j_password)
        
        # Initialize scraper to read cached data
        scraper = None
        if CompatibilityScraper:
            scraper = CompatibilityScraper(db_path=args.compatibility_db, use_llm=False)
            LOGGER.info("Loading cached scraped data from %s", args.compatibility_db)
        
        all_components = [comp for comp_list in components.values() for comp in comp_list]
        
        # Update nodes with scraped attributes
        upsert_components(driver, args.namespace, all_components, scraper=scraper)
        
        # Update typed compatibility edges based on scraped data
        # (Re-run edge building to include scraped compatibility info)
        edge_specs = [
            {"type": "ELECTRICAL_COMPATIBLE_WITH", "edges": electrical_edges},
            {"type": "INTERFACE_COMPATIBLE_WITH", "edges": interface_edges},
            {"type": "SOCKET_COMPATIBLE_WITH", "edges": socket_edges},
        ...
        driver.close()
        LOGGER.info("STEP 3 complete: Graph updated with scraped compatibility data")
    
    LOGGER.info("Knowledge graph build completed successfully.")
    LOGGER.info("Structure: Products are nodes, compatibility relationships are edges, attributes are properties.")

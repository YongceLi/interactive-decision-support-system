...
def compute_coverage_risk_score_max_mode(
    ...
    """
    Compute marginal gain using MAX mode (original implementation).

    Coverage: max over selected vehicles per preference
    Risk: max over selected vehicles per risk signal
    ...
    bonus_gain = soft_bonus[candidate_idx]

    marginal_gain = coverage_gain - lambda_risk * risk_gain + mu * bonus_gain

    return marginal_gain


def compute_coverage_risk_score_sum_mode(
    Q_u: np.ndarray,
    candidate_idx: int,
    Pos: np.ndarray,
    Neg: np.ndarray,
    ...
    """
    Compute marginal gain using SUM mode (proposed noisy-or coverage).

    Coverage gain: Δ_F(v|S) = Σ_u Q_u · g(Pos(u,v))
    Risk gain: Δ_R(v) = Σ_r h(Neg(r,v))  (modular, independent of S)

    Args:
        Q_u: (M,) array of Q_u = Π_{v∈S} (1 - g(Pos(u,v))) for each preference
        candidate_idx: Index of candidate vehicle to add
        Pos: (V, M) matrix of Pos_j(v) scores
        Neg: (V, N) matrix of Neg_j(v) scores
        soft_bonus: (V,) array of soft bonus scores
    ...
    Returns:
        Tuple of (marginal_gain, updated_Q_u after adding candidate)
    """
    # g(Pos(u, v)) for candidate
    g_pos_candidate = g_function(Pos[candidate_idx, :], alpha)

    # Coverage gain: Δ_F(v|S) = Σ_u Q_u · g(Pos(u,v))
    coverage_gain = np.sum(Q_u * g_pos_candidate)

    # Risk gain: Δ_R(v) = Σ_r h(Neg(r,v)) (modular)
    risk_gain = np.sum(h_function(Neg[candidate_idx, :]))

    # Soft bonus gain (modular)
    bonus_gain = soft_bonus[candidate_idx]

    # Marginal gain
    marginal_gain = coverage_gain - lambda_risk * risk_gain + mu * bonus_gain

    # Updated Q_u if this candidate is selected
    # Q_u_new = Q_u · (1 - g(Pos(u, v)))
    updated_Q_u = Q_u * (1 - g_pos_candidate)

    return marginal_gain, updated_Q_u
...
def rank_vehicles_by_alignment(
    ...
    # Step 7: Log detailed alignment for top vehicles
    _log_alignment_details(
        selected_indices[:5],
        ranked_vehicles[:5],
        Pos, Neg, soft_bonus,
...
def _log_alignment_details(
    selected_indices: List[int],
    ranked_vehicles: List[Dict],
    Pos: np.ndarray,
    Neg: np.ndarray,
    soft_bonus: np.ndarray,
    ...
    """Log detailed alignment information for top vehicles."""
    logger.info("=" * 70)
    logger.info(f"FEATURE ALIGNMENT DETAILS (Top {len(ranked_vehicles)}, {mode.value} mode)")
    logger.info("=" * 70)

    for rank, (idx, vehicle) in enumerate(zip(selected_indices, ranked_vehicles), 1):
        logger.info(f"\n#{rank}: {vehicle.get('year')} {vehicle.get('make')} {vehicle.get('model')}")

        # Show soft constraint satisfaction
        if soft_constraints:
        ...
        if liked_features:
            ...
            for j, liked_feature in enumerate(liked_features):
                ...
                if len(vehicle_phrases.pros_phrases) == 0:
                    logger.info(f"    '{liked_feature}' (score: {pos_score:.2f}) → (no phrases)")
                    continue

                # Find best matching phrase
                pros_similarities = liked_embeddings[j] @ vehicle_phrases.pros_embeddings.T
                best_phrase_idx = int(np.argmax(pros_similarities))
                best_sim = float(pros_similarities[best_phrase_idx])
                best_phrase = vehicle_phrases.pros_phrases[best_phrase_idx]
        ...
        if disliked_features:
            ...
            for j, disliked_feature in enumerate(disliked_features):
                ...
                if len(vehicle_phrases.cons_phrases) == 0:
                    logger.info(f"    '{disliked_feature}' (score: {neg_score:.2f}) → (no phrases)")
                    continue

                # Find best matching phrase
                cons_similarities = disliked_embeddings[j] @ vehicle_phrases.cons_embeddings.T
                best_phrase_idx = int(np.argmax(cons_similarities))
                best_sim = float(cons_similarities[best_phrase_idx])
                best_phrase = vehicle_phrases.cons_phrases[best_phrase_idx]
